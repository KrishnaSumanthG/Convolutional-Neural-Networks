...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # shape (N, C).                                                             #
    #############################################################################
    score_prev=X.dot(W1)+b1
    margins=np.maximum(0,score_prev)
    scores=margins.dot(W2)+b2
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    bin[mask,y]-=1
    bin/=N
    grads['W2']=margins.T.dot(bin)
    int1=bin.dot(W2.T)
    int1[margins<=0]=0
    grads['W1']=X.T.dot(int1)
    grads['b1'] = np.sum(int1, axis=0)
    grads['b2'] = np.sum(bin, axis=0)
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      indices=np.random.choice(mask,batch_size,replace=True)
      X_batch = X[indices,:]
      y_batch = y[indices]
      ...
      # v1=mu1*v1-learning_rate*grads['W1']
      # self.params['W1']+=v1
      # v2=mu2*v2-learning_rate*grads['W2']
      # self.params['W2']+=v2
      # v3=mu3*v3-learning_rate*grads['b1']
      # self.params['b1']+=v3
      # v4=mu4*v4-learning_rate*grads['b2']
      # self.params['b2']+=v4
  ...
  def predict(self, X):
    ...
    ###########################################################################
    scores_prev=X.dot(self.params['W1'])+self.params['b1']
    margins=np.maximum(0,scores_prev)
    scores=margins.dot(self.params['W2'])+self.params['b2']
